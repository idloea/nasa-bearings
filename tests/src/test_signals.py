import unittest
from src.signals import calculate_absolute_peak, calculate_average_rectified_value, calculate_crest_factor, calculate_resolution, calculate_root_mean_square, calculate_shape_factor
import numpy as np
...
class TestSignals(unittest.TestCase):
    ...
    def test_calculate_average_rectified_value(self) -> None:
        signal = np.array([-1, 2, -3, 4, -5])
        result = calculate_average_rectified_value(signal)
        expected_result = 3.0
        self.assertEqual(result, expected_result)

    def test_calculate_shape_factor(self) -> None:
        signal = np.array([1, -2, 3, -4, 5])
        result = calculate_shape_factor(signal)
        expected_rms = calculate_root_mean_square(y=signal)
        expected_arv = calculate_average_rectified_value(y=signal)
